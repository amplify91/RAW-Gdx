...
public class HeroProjectile extends Sprite{
	...
	public HeroProjectile(SpriteSheet spriteSheet, GenericPool<HeroProjectile> pool) {
		super(new PhysicsHeroProjectile(), new AnimationHeroProjectile(spriteSheet, WIDTH, HEIGHT), new StateHeroProjectile(), new ControllerNone());
	}
	
	public void create(World world, Sprite parent, Vector2 destinationPoint){
		mPhysics.createBody(world, parent.getPosition().x, parent.getPosition().y, true, true, true, 1, 0, 0);
		mPhysics.createFixture(VERTS_RAW, FixtureType.HERO_PROJECTILE);
